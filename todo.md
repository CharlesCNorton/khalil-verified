...
1. Generalize `rotate_add_mod` to arbitrary naturals, removing the `n ≤ length p` and `m ≤ length p` preconditions.
2. Generalize mutaḥarrik-modifying lemmas (`iḍmār_preserves_morae`, `waqṣ_reduces_morae`, `ʿaṣb_preserves_morae`, etc.) to pattern-general forms analogous to `delete_sakin_preserves_count`.
3. Prove compound zihāf simultaneous deletion equivalent to (or intentionally distinct from) sequential composition.
...
16. Introduce `SuperLong` weight to distinguish tarfīl from tadhyīl.
