...
1. Document that `ʿaṣb_preserves_morae`, `ʿaṣb_reduces_count`, `ʿaql_reduces_morae`, and `ʿaql_reduces_count` cannot be generalized beyond per-foot form — false on arbitrary patterns (e.g. ʿaṣb on [S;S;S;S;S]: morae 5→4, count 5→3; ʿaql on [S;S;L;S;S]: morae 6→4, count 5→3).
2. Prove compound zihāf simultaneous deletion equivalent to (or intentionally distinct from) sequential composition.
...
13. Add meter-specific ḥashw refinements to `foot_permitted_zihaf` (e.g. kaff rare in Ṭawīl).
